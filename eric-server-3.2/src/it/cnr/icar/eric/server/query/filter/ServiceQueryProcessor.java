...
class ServiceQueryProcessor extends RegistryObjectQueryProcessor {
    ...
    protected String processSubQueries() throws RegistryException {
        ...
        List<ServiceBindingQueryType> bindingQueries = ((ServiceQueryType)filterQuery).getServiceBindingQuery();
         
        if (bindingQueries != null) {
         
            Iterator<ServiceBindingQueryType> iter = bindingQueries.iterator();
            while (iter.hasNext()) {

...
public class RelocationManagerImplTest extends ServerTest {
	...
	public void testReAssignOwnerImmediately() throws Exception {
        ...
        roList.getIdentifiable().add(bu.rimFac.createRegistryPackage(testFolder));
        ...
        relocateReq.setId(it.cnr.icar.eric.common.Utility.getInstance().createId());
        
        ObjectRefType ownerAtDest = bu.rimFac.createObjectRefType();
        ownerAtDest.setId(AuthenticationServiceImpl.getInstance().ALIAS_FARRUKH);
        relocateReq.setOwnerAtDestination(ownerAtDest);
        
        AdhocQueryType adhocQuery = bu.createAdhocQueryType("SELECT p.* from RegistryPackage p WHERE p.id='" + testFolderId + "'");

...
public class LifeCycleManagerLocalProxy implements LifeCycleManager {
	...
	private UserType getCallersUser() throws RegistryException {
		...
		if (credentialInfo != null) {
			if (credentialInfo.assertion != null)
				return serverQM.getUser(credentialInfo.assertion);
			
			else if (credentialInfo.cert != null)
				return serverQM.getUser(credentialInfo.cert);
	...
	protected UserType checkRegisterUser(SubmitObjectsRequest req) throws RegistryException {
		...
		if (credentialInfo != null) {
			try {
				...
				// UserRegistrar.getInstance().registerUser(credentialInfo.cert,req);
				try {
					
					// assertion based registration
					if (credentialInfo.assertion != null) {
					
						// class reflection
						Class<?> clazz = Class.forName("it.cnr.icar.eric.server.saml.SAMLRegistrar");
						Method getInstance = clazz.getMethod("getInstance", new Class[] {});
						Object userRegistrar = getInstance.invoke(null, new Object[] {});

						// object reflection
						Method registerUser = clazz.getMethod("registerUser", new Class [] {Assertion.class});
						Object user = registerUser.invoke(userRegistrar, new Object[] {credentialInfo.assertion});
						return (UserType)user;

					} else if (credentialInfo.cert != null) {

						Class<?> clazz = Class.forName("it.cnr.icar.eric.server.security.authentication.UserRegistrar");
						Method getInstance = clazz.getMethod("getInstance", new Class[] {});
						Object userRegistrar = getInstance.invoke(null, new Object[] {});
						Method registerUser = clazz.getMethod("registerUser", new Class[] { X509Certificate.class,
								SubmitObjectsRequest.class });
						Object user = registerUser.invoke(userRegistrar, new Object[] { credentialInfo.cert, req });
						return (UserType) user;
					}
				} catch (Exception re) {
					// Log error, return null (proceed with normal way)
					log.error(
							CommonResourceBundle.getInstance().getString(
									"message.ExceptionWhenCallingUserRegistrarRegisterUser"), re);
